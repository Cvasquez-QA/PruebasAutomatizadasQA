...
export const eliminarRegistroSiExiste = async (t, gridId, codigo, filtroNth = 0) => {
    const grid = gridId.startsWith('#') ? gridId : `#${gridId}`;
    const filtro = Selector(`${grid} .dx-texteditor-input`).nth(filtroNth);

    await t
        .expect(filtro.with({ timeout: 15000 }).exists).ok(
            `No apareció el filtro del grid ${grid} (¿se abrió el maestro correcto?)`
        )
        .click(filtro)
        .pressKey('ctrl+a')
        .pressKey('backspace')
        .typeText(filtro, codigo)
        .wait(2000);
 ...
 * Usa withExactText para no confundir "Tipos" con "Tipos Tercero".
 */
